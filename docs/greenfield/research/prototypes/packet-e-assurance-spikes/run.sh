...
audit_toolchain() {
  local manifest="$here/toolchain.json"
  local expected actual tool_name command_name output_path
  ...
  [[ -e "$(jq -r '.nix.nixpkgs.storePath' "$manifest")" ]] || return 1
  ...
  while IFS='|' read -r tool_name command_name; do
    ...
    [[ -e "$(jq -r --arg name "$tool_name" '.tools[$name].drvPath' "$manifest")" ]] || return 1
  done <<'TOOLS'
node|node
jq|jq
cue|cue
...
TOOLS
  ...
  jq -r '.tools.lean.libraries[].storePath' "$manifest" |
    while IFS= read -r library_path; do
      [[ -e "$library_path" ]] || exit 1
    done || return 1
...
run_group toolchain audit_toolchain
run_group normalization node "$here/normalization/dependency-audit.mjs" --all
